...
final class RhLeaveController extends AbstractController
{
    ...
    #[Route('/welcome/rh/leaves/ai/comment', name: 'app_rh_leave_ai_comment', methods: ['POST'])]
    #[IsGranted('ROLE_RH')]
    public function aiComment(
        ...
        if (!$this->isCsrfTokenValid('rh_leave_ai_comment', (string) ($payload['_token'] ?? ''))) {
        ...
        $leave = $leaveRequestRepository->findOneByRh($leaveId, $this->getCurrentRhId());
        if ($leave === null) {
            return new JsonResponse(['success' => false, 'message' => 'Demande introuvable dans votre perimetre RH.'], 404);
        }

        if ($leave->getStatus() !== 'ATTENTE') {
            return new JsonResponse(['success' => false, 'message' => 'La demande doit etre en attente pour une suggestion IA.'], 422);
        }

        if ($leave->getRequestCategory() === LeaveRequestService::CATEGORY_EXCEPTION
            && $leave->getWorkflowStatus() !== LeaveRequestService::WORKFLOW_RH_PENDING) {
            return new JsonResponse(['success' => false, 'message' => 'Cette demande exceptionnelle n\'est plus dans l\'etape RH.'], 422);
        }
        ...
        $suggestion = $aiService->generateRhLeaveDecisionComment([
            'action' => $action,
